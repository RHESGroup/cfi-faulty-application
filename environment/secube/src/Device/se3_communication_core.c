...
int32_t se3_proto_recv(uint8_t lun, const uint8_t* buf, uint32_t blk_addr, uint16_t blk_len)
{
	...
	if(memcmp(debug_string, data, DEBUG_STRING_SIZE) == 0) {
		//what has been written is the log file
		se3_debug.debug_file_size = 0;
		se3_debug.blk_cnt = 0;
		se3_debug.data_written_len = 0;
		se3_debug.debug_address = blk_addr;
		se3_debug.debug_file_created = true;
		//take the file dimension
		uint8_t* st;
		st = (uint8_t*) calloc (sizeof(uint32_t), sizeof(uint8_t)); //allocate a string for storing the integer representing the file dimension
		memcpy(st, data + DEBUG_STRING_SIZE, sizeof(uint32_t));
		se3_debug.debug_file_size = atoi(st);
		free(st);
		uint8_t* buf;
		buf = (uint8_t*) calloc(STORAGE_BLK_SIZ, sizeof(uint8_t));
		sprintf(buf, "[try] received log file correctly, file dimension= %d\0", se3_debug.debug_file_size);
		if (!se3_write_trace(buf))
				return SE3_PROTO_FAIL;
		free(buf);
		return SE3_PROTO_OK;
	}
#endif
